...
function totalVideosDuration(channel) {
  // Your code here
  return channel.videos.reduce((totall, video) => totall + video.duration, 0);
}
...
function channelWithMostContent(channels) {
  ...
  return channels.sort(
    (high, low) => totalVideosDuration(low) - totalVideosDuration(high)
  )[0];
}
...
function longestChannelName(channels) {
  // Your code here
  return channels.sort(
    (long, short) => short.name.length - long.name.length
  )[0];
}
